...
#' @export
coorSort <- function(coor, by.strand=FALSE, reverse=c("-1","-")) {

    ## first sort by start<end, independent of strand
    
    ## get ordered start/end, independent of strand
    start <- apply(coor[,c("start","end")], 1, min)
    coor <- coor[order(start),]

    ## then order by chromosomes
    coor <- coor[order(coor[,"chr"]), ]

    ## split by chromosomes
    if ( by.strand ) {
        idx <- which(coor[,"strand"]%in%reverse)
        coor <- rbind(coor[-idx,],
                      coor[idx,])
    }
...
#' @export
coorMerge <- function(coor, type, verb=1) {

    ## fill up empty type
    rm.type <- FALSE
    if ( missing(type) ) {
        coor <- cbind(coor, type="all")
        type <- "type"
        rm.type <- TRUE
    }
    ...
    for ( tp in types ) {
        ...
        ## write bedfile
        scoor <- coor[coor[,type]==tp,]
        bcoor <- coor2bed(scoor, file=bfile, verb=0)
        ...
        mcoor <- bed2coor(ofile, c("chr","start","end","strand"))
        ncoor <- rbind.data.frame(ncoor, cbind(mcoor, tp))
    }
    unlink(c(bfile,ofile))

    colnames(ncoor) <- c("chr","start","end","strand", type)
    if ( rm.type ) ncoor <- ncoor[,-ncol(ncoor)]
...
#' considered to stem from the forward strand.
#' @export
coorUpstream <- function(coor, upstream, reverse=c("-1","-")) {

    ## (only valid for non-circular DNA!!)

    str <- as.character(coor[,"strand"])
    
    rev.str <- which( as.character(coor[,"strand"])%in%reverse)
    frw.str <- which(!as.character(coor[,"strand"])%in%reverse)
    
    ## get ordered start/end, independent of strand
    start <- apply(coor[,c("start","end")], 1, min)
    end   <- apply(coor[,c("start","end")], 1, max)
    ...
    coor[,c("start","end")] <- data.frame(start=start, end=end)
    coor
}
...
#' @export
coorPrune <- function(x, chrL, chr="chr", 
